...
def getDBPN(manifesturl):
    soup = BeautifulSoup(urllib.request.urlopen(manifesturl), 'html.parser')
    ...
    soup1 = BeautifulSoup(urllib.request.urlopen(link), 'html.parser')
    database_pn = soup1.find(attrs={"id": "uid_div"})
    link1 = "http://scmdb" + database_pn.find('a').get('href')

    soup2 = BeautifulSoup(urllib.request.urlopen(link1), 'html.parser')
